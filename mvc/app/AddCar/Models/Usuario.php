...
class Usuario extends Modelo implements JsonSerializable
{
    ...
    public function listarAmigos($id_usuario)
    {
        ...
        $query = "select U.usuario,u.imagen, a.id_usuario
                    from usuarios U
                    inner join usuarios_has_amigos A
                    on U.id = A.id_amigo
                    where A.id_usuario = ?";
    ...
    public function traerUsuariosPorEvento($id_evento)
    {
        $db = DBConnection::getConnection();

        $query = "
            select e.id_evento, ehu.id_usuario, u.usuario, u.imagen
            from eventos e
            inner join eventos_has_usuarios ehu
            on e.id_evento = ehu.id_evento
            inner join usuarios u 
            on ehu.id_usuario = u.id
            where e.id_evento = ?";
